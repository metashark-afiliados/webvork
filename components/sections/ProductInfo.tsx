  ...
  <div className="flex items-center gap-0.5">
    {[...Array(5)].map((_, i) => (
      <DynamicIcon
        key={i}
        name="Star"
        className={cn("h-5 w-5", i < Math.floor(rating) ? "text-yellow-400" : "text-muted-foreground/30")}
...
export function ProductInfo({ content }: ProductInfoProps) {
  ...
  const handleAddToCart = () => {
    logger.info(`[ProductInfo] Añadiendo ${quantity} de "${productData.name}" al carrito.`);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <p className="text-sm font-semibold uppercase tracking-wider text-muted-foreground mb-2">{productData.category}</p>
        <h1 className="text-4xl lg:text-5xl font-bold text-foreground">{productData.name}</h1>

        <div className="flex items-center gap-4 mt-4">
           {productData.rating && <StarRating rating={productData.rating} />}
           <span className="text-3xl font-bold text-primary">
             {new Intl.NumberFormat("it-IT", { style: "currency", currency: "EUR" }).format(productData.price)}
           </span>
        </div>
      </div>

      <TextSection content={description} spacing="compact" prose={false} className="py-0 text-muted-foreground" />

      <Separator />

      <div className="flex flex-col sm:flex-row items-center gap-4">
        <div className="flex items-center gap-2">
          <Label htmlFor="quantity" className="text-sm font-medium">{quantityLabel}</Label>
        ...
        <Button onClick={handleAddToCart} size="lg" className="w-full sm:w-auto flex-1">
       ...
       <div className="text-sm text-center">
        {productData.inventory > 0 ? (
          <p className="text-green-600">Disponibile ({productData.inventory} in magazzino)</p>
        ) : (
