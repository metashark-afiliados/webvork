  ...
  return (
    <div className="flex items-center gap-4 py-4">
      <div className="relative h-16 w-16 rounded-md overflow-hidden border">
        <Image src={item.imageUrl} alt={item.name} fill className="object-contain" sizes="64px" />
      </div>
      <div className="flex-1 space-y-1">
        <h4 className="text-sm font-semibold">{item.name}</h4>
        <p className="text-sm text-muted-foreground">{new Intl.NumberFormat("it-IT", { style: "currency", currency: "EUR" }).format(item.price)}</p>
      ...
      <SheetContent className="flex w-full flex-col pr-0 sm:max-w-lg">
        ...
        {items.length > 0 ? (
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-4 text-center">
             <DynamicIcon name="ShoppingCart" className="w-16 h-16 text-muted-foreground/30" />
          ...
          <SheetFooter className="px-6 py-4 bg-muted/50 mt-auto">
            <div className="w-full space-y-4">
              <div className="flex justify-between text-base font-semibold">
                <p>Subtotale</p>
                <p>{new Intl.NumberFormat("it-IT", { style: "currency", currency: "EUR" }).format(cartTotal)}</p>
              </div>
