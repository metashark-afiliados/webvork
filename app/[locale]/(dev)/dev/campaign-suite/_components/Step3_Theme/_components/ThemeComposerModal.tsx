 ...
 * @version 3.0.0 (Full Theme Composer)
...
import { TypographySelector } from './TypographySelector'; // <-- NUEVA IMPORTACIÓN
import { GeometrySelector } from './GeometrySelector';     // <-- NUEVA IMPORTACIÓN
...
interface ThemeComposerModalProps {
    ...
    content: { // <-- NUEVO: Contenido i18n para el modal
...
export function ThemeComposerModal({ isOpen, onClose, fragments, currentConfig, onSave, content }: ThemeComposerModalProps) {
    logger.info("[ThemeComposerModal] Renderizando Compositor de Temas (v3.0 - Full Compositor)");
    ...
    // Limpia la previsualización cuando el modal se cierra
    useEffect(() => {
        if (!isOpen) {
    ...
    const handleSave = () => {
        setPreviewTheme(null); // Limpia la previsualización al guardar
        onSave(localConfig);
        onClose();
    };
    ...
    const assemblePreviewTheme = (config: ThemeConfig): AssembledTheme | null => {
        ...
        // No se requiere que todos estén seleccionados para una previsualización parcial
        // Si no hay presets, solo se usa el base.
        ...
        logger.warn("[ThemeComposer] El tema de previsualización ensamblado es inválido.", { errors: validation.error.flatten() });
        return null;
    }
    ...
    const palettes = Object.entries(fragments.colors).map(([name, data]) => ({
        name,
        colors: (data as AssembledTheme).colors // Aseguramos el tipo para acceder a colors
    }));

    const typographies = Object.entries(fragments.fonts).map(([name, data]) => ({
      name,
      fonts: (data as AssembledTheme).fonts
    }));

    const geometries = Object.entries(fragments.radii).map(([name, data]) => ({
      name,
      geometry: (data as AssembledTheme).geometry
    }));
            ...
            <DialogContent className="max-w-4xl h-[80vh] flex flex-col">
                ...
                <div className="flex-grow overflow-y-auto pr-2">
                    <Tabs defaultValue="colors" className="w-full">
                        <TabsList>
                            <TabsTrigger value="colors">{content.composerColorsTab}</TabsTrigger>
                            <TabsTrigger value="typography">{content.composerTypographyTab}</TabsTrigger>
                            <TabsTrigger value="geometry">{content.composerGeometryTab}</TabsTrigger>
                        </TabsList>
                        <TabsContent value="colors" className="mt-4">
                           <PaletteSelector
                             ...
                             onPreview={(palette) => handlePreviewUpdate({ colorPreset: palette ? palette.name : null })}
                             onCreate={() => toast.info("Funcionalidad de creación de paletas próximamente.")}
                        ...
                        <TabsContent value="typography" className="mt-4">
                           <TypographySelector
                             ...
                             onPreview={(typography) => handlePreviewUpdate({ fontPreset: typography ? typography.name : null })}
                             onCreate={() => toast.info("Funcionalidad de creación de sets de fuentes próximamente.")}
                        ...
                        <TabsContent value="geometry" className="mt-4">
                           <GeometrySelector
                             ...
                             onPreview={(geometry) => handlePreviewUpdate({ radiusPreset: geometry ? geometry.name : null })}
                             onCreate={() => toast.info("Funcionalidad de creación de estilos de radio próximamente.")}
                ...
                <DialogFooter>
                    <Button variant="outline" onClick={onClose}>{content.composerCancelButton}</Button>
